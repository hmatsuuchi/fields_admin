...
function toggleClicksToInstructorContainer(enableDisable) {
    let instructorSelectContainer = document.getElementById('instructor-select-container');
...
function buildInstructorFilterMenu() {
    ...
    let instructorFilterButton = buildElement('div', 'toolbar-button', 'instructor-filter-button', '');
    instructorFilterButton.addEventListener('click', toggleInstructorFilterMenu);
    buttonGroup.appendChild(instructorFilterButton);

    let instructorSelectContainer = buildElement('div', '', 'instructor-select-container', '');
    buttonGroup.appendChild(instructorSelectContainer);

    let instructorSelectTitle = buildElement('div', 'button-sub-menu-title', '', '講師');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            let button = buildElement('div', 'instructor-select-button', 'no-instructor-filter-button', '')
            ...
            for (i = 0; i < instructors.length; i++) {
                ...
                button.addEventListener('click', toggleInstructorFilter);
                if (data.activeInstructors && data.activeInstructors.includes(instructors[i].fields.user)) {
                    button.classList.add('selected');
                }
                instructorSelectContainer.appendChild(button);
            }
...
function toggleInstructorFilter() {
    ...
    // adds active class to instructor select button if instructor filter is active
    let instructorSelectContainer = document.getElementById('instructor-select-container');
    let instructorFilterButton = document.getElementById('instructor-filter-button');
    if (instructorSelectContainer.getElementsByClassName('selected').length > 0) {
        instructorFilterButton.classList.add('active');
    } else {
...
function buildDateNavigation() {
    ...
    let previousDateArrow = buildElement('div', 'date-arrow', 'previous-date-arrow', '');
    ...
    if (dateParam) {
        ...
        if (today.getFullYear() == actualToday.getFullYear() && today.getMonth() == actualToday.getMonth() && today.getDate() == actualToday.getDate()) {
            dayOfWeekLabel += ' (今日)';
        };
    } else {
        today = new Date();
...
function fetchAttendanceData() {
    ...
    $.ajax({
        ...
        data: parameters,
        success: function (data) {
            let expandedViewActive = document.getElementById('page-meta').dataset.expanded_view_active;

            if (expandedViewActive == 'True') {
                dataBuildRecordsExpanded(JSON.parse(data.allRecords), JSON.parse(data.allClasses), data.classTypeLookupList, JSON.parse(data.allInstructors), JSON.parse(data.allStudents), JSON.parse(data.allAttendance), data.allInstructorChoices);
            } else {
                dataBuildRecordsCompact(JSON.parse(data.allRecords), JSON.parse(data.allClasses), JSON.parse(data.allAttendance), JSON.parse(data.allStudents));
            }
...
function buildAddNewAttendanceTop() {
    ...
    let addNewAttendanceContainer = buildElement('div', 'add-new-attendance-container', '', '');
...
function addNewAttendanceRecord() {
    ...
    // PRIMARY CONTAINER
    let newAttendanceContainer = buildElement('div', 'attendance-container', '', '');

    let allAttendanceCards = attendanceSection.getElementsByClassName('attendance-container');
    if (allAttendanceCards.length == 0) {
        attendanceSection.appendChild(newAttendanceContainer);
    } else {
        attendanceSection.insertBefore(newAttendanceContainer, allAttendanceCards[0]);
    }
    ...
    let attendanceDate = buildElement('div', 'attendance-date', '', `${attendanceDateBrowse}（${dayOfWeekConvertShort[attendanceDateAsDate.getDay()]}）`);
    ...
    let attendanceRecordContainer = buildElement('div', 'attendance-record-container', '', '');
    ...
    let attendanceRecordItem = buildElement('div', 'attendance-record-item', '', '');
    ...
    let addNewStudentButton = buildElement('div', 'plus-button-container', '', '');
    ...
    let addNewStudentButtonVertical = buildElement('div', 'plus-icon-bar', '', '');
    ...
    let addNewStudentButtonHorizontal = buildElement('div', 'plus-icon-bar', '', '');
    ...
    let addNewStudentButtonText = buildElement('div', 'add-new-student-button', '', '生徒追加');
...
function buildClassSelect() {
    let newClassSelectContainer = document.getElementById('new-class-select-container');

    // SEARCH INPUT CONTAINER
    let searchInputContainer = buildElement('div', '', 'class-search-input-container', '');
    ...
    let searchInput = buildElement('input', 'search-input-field', 'class-search-input', '');
    ...
    let resultsContainer = buildElement('div', 'results-container', 'class-results-container', '');
...
function buildClassCards(classes) {
    ...
    for (i = 0; i < classes.length; i++) {
        ...
        // CLASS CONTAINER
        let currentClassContainer = buildElement('div', 'class-container', `class-${currentClass.id}`, '');
        ...
        let currentClassHeader = buildElement('div', 'class-header', '', currentClass.className);
        ...
        let currentClassDayOfWeekContainer = buildElement('div', 'day-of-week-container', '', '');
        ...
        var n;
        for (n = 0; n < 7; n++) {
            let currentClassDay = buildElement('div', 'day', '', `${dayOfWeekText[n]}`);
        ...
        let currentClassTimeContainer = buildElement('div', 'time-container', '', '');
        ...
        let primaryInstructorIcon = buildElement('div', 'primary-instructor-icon', '', '');
        ...
        let classDescriptionContainer = buildElement('div', 'class-description-container', '', '');
        ...
        let primaryInstructor = buildElement('div', 'instructor', '', `${classes[i].primaryInstructorLastName}先生`);
        ...
        for (x = 0; x < classes[i].students.length; x++) {
            ...
            let studentNameContainer = buildElement('a', 'student-name-container', '', '');
            ...
            let studentNameKanji = buildElement('a', 'student-name-kanji', '', `${classes[i].students[x].lastNameKanji} ${classes[i].students[x].firstNameKanji}`);
            ...
            let studentNameKatakana = buildElement('a', 'student-name-katakana', '', `${classes[i].students[x].lastNameKatakana} ${classes[i].students[x].firstNameKatakana}`);
...
function newClassButtonClick() {
    ...
    let allAttendanceContainers = document.getElementsByClassName('attendance-container');
    var n;
    ...
    if (this.parentElement.parentElement.classList.contains('attendance-container')) {
        ...
        let targetAttendanceContainer = this.parentElement.parentElement.parentElement;
        targetAttendanceContainer.classList.add('target-attendance-container');
    }
    ...
    let classResultsContainer = document.getElementById('class-results-container');
...
function searchInputKeyUp() {
    ...
    for (i = 0; i < allResults.length; i++) {
        ...
        if (inputRaw.length == 0) {
            currentClass.classList.add('class-container-active');
        } else if (classNameLower.indexOf(inputLower) != -1 | instructorLower.indexOf(inputLower) != -1 | studentLower.indexOf(inputLower) != -1) {
            currentClass.classList.add('class-container-active');
...
function addClassToAttendance() {
    ...
    let targetAttendanceContainer = document.getElementsByClassName('target-attendance-container')[0]; // target class container

    // adds cloned class card to attendance
    let addedClassCardContainer = targetAttendanceContainer.getElementsByClassName('added-class-card')[0];
    ...
    let addClassButton = targetAttendanceContainer.getElementsByClassName('add-new-class-button')[0];
    if (addClassButton) {
    ...
    let attendanceTimeInput = targetAttendanceContainer.getElementsByClassName('class-time-input')[0]; // attendance record input
    ...
    let allChoices = targetAttendanceContainer.getElementsByClassName('instructor-input')[0].children;
    var i;
    ...
    let allStudentsContainer = classCard.getElementsByClassName('students-container')[0]; // get all student container
    let allStudents = allStudentsContainer.children; // gets individual student records

    let attendanceRecordContainer = targetAttendanceContainer.getElementsByClassName('attendance-record-container')[0]; // attendance record container

    // removes incompleted attendance records
    var removedRecords = [];
    const elements = attendanceRecordContainer.getElementsByClassName('incomplete-record');
    ...
    for (i = 0; i < allStudents.length; i++) {
        if (!presentRecordsIDList.includes(allStudents[i].dataset.student_id)) { // checks to see if record already exists
            // ATTENDANCE RECORD CONTAINER
            let newStudentAttendanceRecord = buildElement('div', 'attendance-record-item', '', '')
            newStudentAttendanceRecord.classList.add('added-student');
            newStudentAttendanceRecord.classList.add('incomplete-record');
            newStudentAttendanceRecord.dataset.student_id = allStudents[i].dataset.student_id;
            let addStudentButton = attendanceRecordContainer.getElementsByClassName('add-student-container')[0];
            newStudentAttendanceRecord.classList.add('pointer-events-none'); // disables pointer events while DB is updated
            attendanceRecordContainer.insertBefore(newStudentAttendanceRecord, addStudentButton);

            // ATTENDANCE RECORD CONTAINER -> REMOVE STUDENT BUTTON CONTAINER
            let removeStudentButtonContainer = buildElement('div', 'remove-student-button-container', '', '');
            removeStudentButtonContainer.addEventListener('click', removeStudentAttendanceRecord);
            ...
            let removeStudentButtonBar = buildElement('div', 'remove-student-button-bar', '', '');
            ...
            nameKanji.innerHTML = allStudents[i].getElementsByClassName('student-name-kanji')[0].innerHTML;
            ...
            nameKatakana.innerHTML = allStudents[i].getElementsByClassName('student-name-katakana')[0].innerHTML;
            newStudentAttendanceRecord.appendChild(nameKatakana);

            // ATTENDANCE RECORD CONTAINER -> ATTENDANCE STATUS CONTAINER
            let attendanceStatusContainer = buildElement('div', 'attendance-status-container', '', '')
    ...
    let allStudentsAttendance = classCard.getElementsByClassName('student-name-container');
    let studentIDList = []
    ...
    $.ajax({
        ...
        data: parameters,
        success: function (data) {
            let newStudentAttendanceRecordsAll = targetAttendanceContainer.getElementsByClassName('incomplete-record');
            var i;
            for (i = 0; i < newStudentAttendanceRecordsAll.length; i++) {
                newStudentAttendanceRecordsAll[i].dataset.record_id = data.studentAttendanceIDList[i];
                newStudentAttendanceRecordsAll[i].classList.remove('pointer-events-none'); // enables pointer events after DB is updated
...
function newStudentButtonClick() {
    ...
    var i;
    for (i = 0; i < allContainers.length; i++) { // removes 'target-attendance-container' class from all attendance containers on page
    ...
    let studentSelectContainer = document.getElementById('new-student-select-container'); // student select container
    ...
    let studentsResultsContainer = document.getElementById('student-results-container');
...
function buildStudentSelect() {
    let newStudentSelectContainer = document.getElementById('new-student-select-container');

    // SEARCH INPUT CONTAINER
    let searchInputContainer = buildElement('div', 'search-input-container', 'student-search-input-container', '');
    newStudentSelectContainer.appendChild(searchInputContainer);

    // SEARCH INPUT CONTAINER -> MAGNIFYING GLASS
    let magnifyingGlass = buildElement('div', 'magnifying-glass', 'magnifying-glass', '');
    searchInputContainer.appendChild(magnifyingGlass);

    // SEARCH INPUT CONTAINER -> SEARCH INPUT
    let searchInput = buildElement('input', 'search-input-field', 'student-search-input', '');
    ...
    // SEARCH INPUT CONTAINER -> RESULTS CONTAINER
    let resultsContainer = buildElement('div', 'results-container', 'student-results-container', '');
...
function studentSearchInputKeyUp() {
    let allStudents = document.getElementById('student-results-container').children; // all choices
...
function newStudentOverlayClose() {
    ...
    let studentSelectContainer = document.getElementById('new-student-select-container'); // student select container
    studentSelectContainer.classList.remove('new-student-select-container-active'); // removes class to display container and contents
}
...
function buildStudentCards(resultsContainer, students) {
    ...
    for (i = 0; i < students.length; i++) {
        ...
        let studentItemContainer = buildElement('div', 'student-item-container', `student-item-${students[i].id}`, '');
        ...
        let studentItemKanji = buildElement('div', 'student-item-kanji', '', `${students[i].lastNameKanji} ${students[i].firstNameKanji}`);
        studentItemContainer.appendChild(studentItemKanji);

        // STUDENT ITEM CONTAINER -> KATAKANA
        let studentItemKatakana = buildElement('div', 'student-item-katakana', '', `${students[i].lastNameKatakana} ${students[i].firstNameKatakana}`);
...
function selectStudent() {
    ...
    let attendanceContainer = document.getElementsByClassName('target-attendance-container')[0];
    let recordContainer = attendanceContainer.getElementsByClassName('attendance-record-container')[0];
    let addStudentContainer = attendanceContainer.getElementsByClassName('add-student-container')[0];

    let studentID = this.dataset.student_id;

    let allCurrentRecords = recordContainer.getElementsByClassName('added-student');
    var i;
    var noMatch = true;
    ...
    if (noMatch) {
        let nameKanji = this.getElementsByClassName('student-item-kanji')[0].innerHTML;
        if (this.dataset.grade != '') {
            nameKanji += `（${this.dataset.grade}）`;
        }
        let nameKatakana = this.getElementsByClassName('student-item-katakana')[0].innerHTML;
        ...
        let studentNameKanji = buildElement('div', 'student-name-kanji', '', nameKanji);
        recordItem.appendChild(studentNameKanji);
    
        // ATTENDANCE RECORD CONTAINER -> REMOVE STUDENT BUTTON CONTAINER -> KATAKANA
        let studentNameKatakana = buildElement('div', 'student-name-katakana', '', nameKatakana);
    ...
    let studentSelectContainer = document.getElementById('new-student-select-container');
    studentSelectContainer.classList.remove('new-student-select-container-active');
...
function buildConfirmationWindow() {
    ...
    let confirmationText = buildElement('div', '', 'confirmation-text', 'このレコードを削除しますか？');
    confirmationBox.appendChild(confirmationText);

    // CONFIRMATION BOX -> CONFIRM
    let confirmationConfirm = buildElement('div', 'confirmation-box-button', 'confirmation-confirm', 'はい、大丈夫です');
    ...
    let confirmationReject = buildElement('div', 'confirmation-box-button', 'confirmation-reject', '戻る');
...
function dataBuildRecordsExpanded(recordsAll, classesAll, classTypeLookupList, instructorsAll, studentsAll, attendanceAll, allInstructorChoices) {
    let csrfToken = document.getElementById('page-meta').dataset.csrf;
    ...
    for (i = 0; i < recordsAll.length; i++) {
        ...
        let classData = classesAll.find(o => o.pk === recordsAll[i].fields.linked_class); // get associated class data
        ...
        if (classData) {
            ...
            let classCardContainer = buildElement('div', 'class-container', `class-${classData.pk}`, '');
            ...
            let classHeader = buildElement('div', 'class-header', '', classData.fields.class_name);
            ...
            let dayOfWeekContainer = buildElement('div', 'day-of-week-container', '', '');
            ...
            for (n = 0; n < 7; n++) {
                let currentClassTime = buildElement('div', 'time', '', '');
                if (classData.fields.day_of_week == n && classData.fields.start_time != '') {
                    currentClassTime.classList.add('time-active');
            ...
            let insructorData = instructorsAll.find(o => o.fields.user === classData.fields.primary_instructor); // finds instructor in instructor list
            let instructorName = insructorData.fields.last_name_kanji; // gets instructor last name in kanji
            let instructor = buildElement('div', 'instructor', '', `${instructorName}先生`);
            ...
            for (y = 0; y < classData.fields.students.length; y++) {
                studentData = studentsAll.find(o => o.pk === classData.fields.students[y]);
                studentObject = {
            ...
            for (p = 0; p < classData.fields.students.length; p++) {
                // STUDENT OBJECT
                let student = studentsAll.find(o => o.pk === classData.fields.students[p]);

                // STUDENT NAME CONTAINER
                let studentNameContainer = buildElement('div', 'student-name-container', '', '');
                studentsContainer.appendChild(studentNameContainer);

                // STUDENT NAME CONTAINER -> ENROLLMENT STATUS INDICATOR
                let enrollmentStatusIndicator = buildElement('a', 'enrollment-status-indicator', '', '');
                enrollmentStatusIndicator.classList.add(enrollmentStatusText[student.fields.status]);
                ...
                let studentNameKanji = buildElement('a', 'student-name-kanji', '', studentNameKanjiFormatted);
                studentNameContainer.appendChild(studentNameKanji);
                ...
                let studentNameKatakana = buildElement('a', 'student-name-katakana', '', studentNameKatakanaFormatted);
                studentNameContainer.appendChild(studentNameKatakana);
            }
        } else {
        ...
        attendanceString = `${attendanceYear}-${attendanceMonth.slice(-2)}-${attendanceDay.slice(-2)}`;

        let attendanceDate = buildElement('div', 'attendance-date', '', `${attendanceString}（${dayOfWeekConvertShort[attendanceDateAsDate.getDay()]}）`);
        dateTimeContainer.appendChild(attendanceDate);
        ...
        let relatedAttendance = attendanceAll.filter(o => o.fields.attendance_record === recordsAll[i].pk);

        // ATTENDANCE SORT
        relatedAttendance.sort((a, b) => {
            let aLastName = studentsAll.find(o => o.pk === a.fields.student).fields.last_name_katakana;
            let aFirstName = studentsAll.find(o => o.pk === a.fields.student).fields.first_name_katakana;
            let bLastName = studentsAll.find(o => o.pk === b.fields.student).fields.last_name_katakana;
            let bFirstName = studentsAll.find(o => o.pk === b.fields.student).fields.first_name_katakana;
        ...
        for (x = 0; x < relatedAttendance.length; x++) {
            ...
            let associatedStudent = studentsAll.find(o => o.pk === relatedAttendance[x].fields.student);

            // STUDENT ATTENDANCE CONTAINER
            let attendanceRecordItem = buildElement('div', 'attendance-record-item', '', '');
            attendanceRecordItem.classList.add('added-student');
            attendanceRecordItem.classList.add(statusCSS[relatedAttendance[x].fields.attendance_status]);
            ...
            if (associatedStudent.fields.grade != 0) {
                studentNameKanjiFormatted += `（${gradeText[associatedStudent.fields.grade]}）`;
            }
...
function dataBuildRecordsCompact(recordsAll, classesAll, attendanceAll, studentsAll) {
    let csrfToken = document.getElementById('page-meta').dataset.csrf;
    ...
    for (i = 0; i < recordsAll.length; i++) {
        ...
        if (classData) {
            let classContainer = buildElement('div', 'class-container', `class-${classData.pk}`, '');
            attendanceContainer.appendChild(classContainer);
        }

        // CONTAINER - INSTRUCTOR ICON
        if (recordsAll[i].fields.instructor) {
            let instructorIcon = buildElement('div', 'primary-instructor-icon', '', '');
        ...
        if (recordsAll[i].fields.start_time || classData) {
            ...
            if (recordsAll[i].fields.start_time) {
                let time = buildElement('div', 'section-time', '', `${recordsAll[i].fields.start_time.slice(0, 5)}`)
                titleContainer.appendChild(time);
            }

            // CONTAINER - TITLE CONTAINER - TIME
            if (classData) {
                let className = buildElement('div', 'class-name', '', classData.fields.class_name);
                titleContainer.appendChild(className);
            }
        }

        // CONTAINER - ATTENDANCE RECORD DATA
        let attendanceRecordData = attendanceAll.filter(o => o.fields.attendance_record === recordsAll[i].pk)
        ...
        for (n = 0; n < attendanceRecordData.length; n++) {
            ...
            let recordItem = buildElement('div', 'attendance-record-item', '', '');
            recordItem.classList.add('added-student');
            recordItem.classList.add(statusCSS[attendanceRecordData[n].fields.attendance_status]);
            ...
            let studentData = studentsAll.find(o => o.pk === attendanceRecordData[n].fields.student);
            ...
            if (studentData.fields.grade != 0) {
                formattedText = formattedText.concat(`（${gradeText[studentData.fields.grade]}）`);
            }
            let studentNameKanji = buildElement('a', 'student-name-kanji', '', formattedText);
            studentNameKanji.href = `/customer/detail/${studentData.pk}`;
            recordItem.appendChild(studentNameKanji);

            // CONTAINER - STUDENT NAME KATAKANA
            let studentNameKatakana = buildElement('a', 'student-name-katakana', '', `${studentData.fields.last_name_katakana} ${studentData.fields.first_name_katakana}`);
...
function toggleExpandedView() {
    let expandedViewActive = document.getElementById('page-meta').dataset.expanded_view_active; // expanded view active meta data
    let expandedViewButton = document.getElementById('toggle-expanded-view-button'); // button
...
function buildProgressTrackingElement() {
    ...
    let incompleteCount = buildElement('div', 'record-count-bubble', 'incomplete-record-count', '0');
    primaryContainer.appendChild(incompleteCount);

    // PRIMARY CONTAINER -> PRESENT RECORD COUNT
    let presentCount = buildElement('div', 'record-count-bubble', 'present-record-count', '0');
    primaryContainer.appendChild(presentCount);

    // PRIMARY CONTAINER -> ABSENT RECORD COUNT
    let absentCount = buildElement('div', 'record-count-bubble', 'absent-record-count', '0');
...
function progressCalculate() {
    ...
    let totalRecords = incompleteRecords.length + presentRecords.length + absentRecords.length; // total records present on page
    ...
    if (totalRecords > 0) {
        percentageComplete = (presentRecords.length + absentRecords.length) / totalRecords * 100; // calculates completion percentage
    ...
    if (percentageComplete == 100) {
        section.getElementsByClassName('primary-container')[0].classList.add('attendance-completed');
    } else {
        section.getElementsByClassName('primary-container')[0].classList.remove('attendance-completed');
    }
...
function buildWebsocketStatus() {
    ...
    let loadingDotsContainer = buildElement('div', 'loading-dots', 'ws-loading-dots-container', '');
...
function addWSEventListeners() {
    let attendanceButtonsAll = document.getElementsByClassName('attendance-status-container');
    var i;
...
function openWebsocket() {
    ...
    attendanceWebSocket.onmessage = function(e) {        
        ...
        function removeAttendanceStatusClasses(attendanceRecord) {
            attendanceRecord.classList.remove('incomplete-record', 'present-record', 'absent-record');
        }

        let allItems = document.getElementsByClassName('attendance-record-item');
        var i;
        for (i = 0; i < allItems.length; i++) {
            if (allItems[i].dataset.record_id == data.attendanceID && data.attendanceStatus == 0) {
                removeAttendanceStatusClasses(allItems[i]);
                allItems[i].classList.add('incomplete-record');
            } else if (allItems[i].dataset.record_id == data.attendanceID && data.attendanceStatus == 1) {
                removeAttendanceStatusClasses(allItems[i]);
                allItems[i].classList.add('present-record');
            } else if (allItems[i].dataset.record_id == data.attendanceID && data.attendanceStatus == 2) {
                removeAttendanceStatusClasses(allItems[i]);
...
function transmitWSAction() {
    ...
    attendanceWebSocket.send(JSON.stringify({
